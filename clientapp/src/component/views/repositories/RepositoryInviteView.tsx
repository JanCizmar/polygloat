            ...
            <BaseView title="Invite user" xs={12} md={8} lg={6}>
                {() => (
                    <>
                        ...
                        {state.invitationsLoad.loading && <BoxLoading/> || (!!state.invitationsLoad.data.length &&
                            <Box mt={4}>
                                <Typography variant="h6">Active invitation codes</Typography>
                                <List>
                                    {state.invitationsLoad.data.map(i => (
                                        <ListItem key={i.id}>
                                            <ListItemText>
                                            ...
                                            <ListItemSecondaryAction>
                                                <Button color="secondary" onClick={() => actions.delete.dispatch(i.id)}>Cancel</Button>
                                            </ListItemSecondaryAction>
                                        </ListItem>
                                    ))}
                                </List>
